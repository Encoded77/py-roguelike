...
def fireball_scroll(x, y):
    item_component = Item(use_function=cast_fireball, targeting=True, targeting_message=Message(
        'Left-click a target tile for the fireball, or right-click to cancel.', libtcod.light_red), damage=12, radius=3)
...
def confusion_scroll(x, y):
    ...
    return Entity(x, y, '#', libtcod.light_cyan, 'Confusion scroll', render_order=RenderOrder.ITEM, 
                  item=item_component)
